...
import { setCaseFile, saveCaseFile, getCaseFile, setCase } from '../../../../store/reducers/agentReducer';
...
const CaseFile = ({ navigation }) => {
    ...
    const save = () => {
        setLoading(true)
        dispatch(saveCaseFile({...agent.case, caseId:agent.selectedCase.caseId})).then(()=>{
            setLoading(false)
            navigation.goBack()
        })
    }
    ...
    useEffect(()=>{
        if(agent.selectedCase?.caseId){
            dispatch(getCaseFile(agent.selectedCase.caseId)).then((res)=>{
                dispatch(setCase(res.payload));
            })
        }
                ...
                <BottomButtonContainer>
                    {agent.action !== actions.ADD_FILE && (
                        <Button
                            ...
                            borderRadius={15}
                            onPress={() =>
                                navigation.navigate(Routes.UPLOADPATIENTFILE)
                            }
                        />
                    )}
                    {agent.action !== actions.DETAIL_FILE && (
